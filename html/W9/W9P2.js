let W9P2 = {
    ...
    initObject: (gl, program) => {
        ...
        program.render = (P, V, lPos) => {

            gl.useProgram(program);
            
            W9P2.initAttributeVariable(gl, program.a_Position, program.vBuffer);
            W9P2.initAttributeVariable(gl, program.a_Normal, program.nBuffer);
            W9P2.initAttributeVariable(gl, program.a_Color, program.cBuffer);
    ...
    initGround:(gl, program) => {
        ...
        // Vertices
        let GroundQuad = [vec3(2,W9P2.Yg,-5),vec3(2,W9P2.Yg,-1),vec3(-2,W9P2.Yg,-5),vec3(-2,W9P2.Yg,-1)];
        program.vBuffer = gl.createBuffer();
        program.vBuffer.num = 3;
        program.vBuffer.type = gl.FLOAT;
        gl.bindBuffer(gl.ARRAY_BUFFER, program.vBuffer);
        ...
        program.render = (P, V) => {
            gl.useProgram(program);
            W9P2.initAttributeVariable(gl, program.a_Position, program.vBuffer);
            W9P2.initAttributeVariable(gl, program.a_Texcord, program.tBuffer);
    ...
    init: () => {
        ...
        function render(){
            setTimeout(function(){
                ...
                let P = perspective(90, 2, 0.01, 10);
                
                groundProgram.render(P, V);
                objectProgram.render(P, V);
            }, 25);
        }
